...
preps::cpufreq::set_governor() {
	...
	if [[ "${governor}" != "conservative" && "${governor}" != "ondemand" && "${governor}" != "performance" && "${governor}" != "powersave" && "${governor}" != "userspace" ]]; then
		main::log_event -level "FATAL" -message "Invalid Governor Value: [${governor}]"
	fi
	if [[ "$(apis::cpupower::get_governor)" != "${governor}" ]]; then
		if apis::cpupower::set_governor -governor "${governor}"; then
			main::log_event -level "INFO" -message "Set CPU Governor: [${governor}]"
		else
			main::log_event -level "ERROR" -message "Failed to Set CPU Governor: [${governor}] - Return Code: [$?]"
			rc=1
		fi
	else
