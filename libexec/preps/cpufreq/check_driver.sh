...
preps::cpufreq::check_driver() {
	...
	local -i rc=0
	local num_cpus="${1}"
	[[ -n "${num_cpus}" ]] || main::log_event -level "FATAL" -message "Missing Argument: [Number of CPUs]"
	(( num_cpus > 0 )) || main::log_event -level "FATAL" -message "Invalid Number of CPUs: [${num_cpus}]"
