...
preps::nvidia::set_power_limit() {
  ...
  [[ -n "${power_limits}" ]] ||
    { main::log_event -level "ERROR" -message "Missing Argument: [Power Limits]"; return ${rc}; }
  local -i index
  for index in 1 0; do
    (( ${power_limits[index]} > 0 )) ||
      { main::log_event -level "ERROR" -message "Invalid Power Limit Value: [${power_limits[index]}] for Scope: [${index}/${scope[index]}]"; return ${rc}; }
  done
  (( ${power_limits[1]} >= ${power_limits[0]} )) ||
    { main::log_event -level "ERROR" -message "Power Limit: [${power_limits[1]}] for Scope: [1/${scope[1]}] Must Be Greater or Equal than Power Limit: [${power_limits[0]}] for Scope: [0/${scope[0]}]"; return ${rc}; }
  for index in 1 0; do
