...
preps::nvidia::set_vboost_slider() {
  ...
  local vboost="${1}"
  [[ -n "${vboost}" ]] ||
    { main::log_event -level "ERROR" -message "Missing Argument: [Video Boost Slider]"; return ${rc}; }
  (( vboost >= 0 && vboost <= 4 )) ||
    { main::log_event -level "ERROR" -message "Invalid Video Boost Slider Value: [${vboost}]"; return ${rc}; }
