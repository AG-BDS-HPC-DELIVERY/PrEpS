...
|CPU Frequency |Manage CPU frequency
...
Example:
----
--comment="NVIDIA_POWER_LIMIT=680;NVIDIA_VBOOST_SLIDER=1"
----
====
...
|
|`PREPS_SKIP_PROLOG_SLURMD`
|Skip Execution of Slurmd Prolog: {no \| yes}

|CPU Frequency
|`CPUFREQ_FREQUENCY_MAX`
|CPU Frequency Max. in KHz (default) or MHz / GHz

|
|`CPUFREQ_FREQUENCY_MIN`
|CPU Frequency Min. in KHz (default) or MHz / GHz
...
|NVIDIA
|`NVIDIA_APPLICATIONS_CLOCKS`
|Applications Clocks in MHz: MEMORY,GRAPHICS

|
|[`NVIDIA_POWER_LIMIT_GPU`:]`NVIDIA_POWER_LIMIT_MODULE`
|Power Limit of the CG Module/GPU Devices in Watts

|
|`NVIDIA_VBOOST_SLIDER`
|Video Boost Slider: {0 \| 1 \| 2 \| 3 \| 4}
